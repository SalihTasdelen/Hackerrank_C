 ...
 *       Filename:  main.c
 *
 *    Description:  Write three variadic functions sum(),max(),min(). 
 *
 *        Version:  1.0
 *        Created:  02-09-2020 13:58:44
 *       Revision:  none
 *       Compiler:  gcc
 *
...
int min(int count,...) {
    va_list arg_pointer;
    va_start(arg_pointer, count);
    int min = va_arg(arg_pointer, int);
    int temp;
    for (int i = 1; i < count; i++){
        if ((temp = va_arg(arg_pointer, int)) < min){
            min = temp;
        }
    }
    return min;
...
int max(int count,...) {
    va_list arg_pointer;
    va_start(arg_pointer, count);
    int max = va_arg(arg_pointer, int);
    int temp;
    for (int i = 1; i < count; i++){
        if((temp = va_arg(arg_pointer, int)) > max){
            max = temp;
        }
    }
    return max;
